...
class Auth0Strategy extends JWTStrategy {
  ...
  verifyConfiguration () {
    ...
    const allowedKeys = [ 'domain', 'entity', 'entityId', 'header', 'keysService', 'schemes', 'service' ];
  ...
  async getEntity (user_id, params) {
    const { entity, entityId } = this.configuration
    ...
    const results = await entityService.find(params)
  ...
  async authenticate(authentication, params) {
    ...
    // get the jwksUri and domain for verifying JWTs

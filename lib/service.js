...
const connection = require('./hooks/connection')
const events = require('./hooks/events')
    ...
    for (let svc of svcs) {
      if (svc !== '/authentication') {
        this.app.service(svc).hooks({
          before: {
            all: [
              unless(some(isProvider('server'), isProvider('socketio'), fromAuth0()), authenticate('auth0'))
            ]
          }
        })
      }

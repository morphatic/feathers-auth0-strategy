...
module.exports = ({
  ...
  authorizeRest = async (context) => {
    ...
    if (context.type !== 'before')
      throw new NotAuthenticated('`authorize()` can only be used as a `before` hook.', context)
    ...
    try {
      ...
      throw new NotAuthenticated('No user with this ID exists.', context)
    }
  ...
  auth0SocketIO = app => {
     ...
     */
    const disconnectWithError = (socket, err, code) => {
      code = code || 'unknown'
    ...
    app.io.sockets.on('connection', socket => {
      ...
      socket.on('authenticate', payload => {
        ...
        const token = jwt.decode(payload.token, { complete: true })
